...
- `GOOGLE_CHAT_CREDENTIALS`: optional Google Chat app service account JSON when `googlechatbot.enabled=true`
- `GOOGLECHATBOT_API_KEY`: API key the Googlechatbot uses to call Centaur; generated by local bootstrap when `GOOGLE_CHAT_CREDENTIALS` is present and this is omitted
