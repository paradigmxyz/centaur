...
def test_default_backend_is_parallel(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(client_module.WEBSEARCH_BACKEND_ENV, raising=False)
    configure(StubBackend())

    client = WebSearchClient()

    assert client.backend_name == "parallel"
    assert isinstance(client._backend, ParallelBackend)
