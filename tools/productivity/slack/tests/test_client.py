...
def test_send_app_dm_calls_centaur_api(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client, fake_web_client = _make_client()
    client._format_requester_attribution = lambda: "\n\n_(requested by <@UREQUESTER>)_"  # type: ignore[method-assign]
    ...
    def fake_urlopen(req, *args, **kwargs):
        ...
        body = json.dumps(
            {"ok": True, "channel": "D12345678", "ts": "123.456"}
        ).encode()
...
def test_send_app_dm_rejects_invalid_user_id() -> None:
    ...
    with pytest.raises(ValueError, match="Slack user ID"):
        client.send_app_dm("#general", "hello")


def test_api_only_client_does_not_require_slack_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("slack.client.secret", lambda _name, default="": default)

    client = SlackClient(allow_missing_bot_token=True)

    assert client.token == ""


def test_send_app_dm_wrapper_does_not_require_slack_token(
    ...
    monkeypatch.setattr(
        SlackClient,
        "send_app_dm",
        lambda _self, user_id, text: {"user_id": user_id, "text": text},
    )

    assert slack_client_module.send_app_dm("U12345678", "hello") == {
        "user_id": "U12345678",
        "text": "hello",
    }
