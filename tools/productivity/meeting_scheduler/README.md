...
aliases. Meetings are always created on a deployment-managed calendar, and
requesters can be included only as ordinary attendees. Centaur-created events
allow guests to modify event details and invite others, giving participants
...
All meetings are owned by the Zoom account bound to the OAuth credential.
Centaur never uses `schedule_for` or another user's identity to create a room.
Zoom requests use the OAuth identity-bound `/users/me` endpoint, so that account
...
Zoom ownership remains with the OAuth-bound service account. Requester identity
is never sent to Zoom as an owner, scheduler, or alternative host. The
confirmation-gated `end_meeting` operation remains available as a fallback and
lets an authorized workflow ask Centaur to end its own Zoom meeting through the
provider status API. It only accepts a recorded Centaur occurrence and
deliberately keeps the Calendar event intact so recording processing and
post-meeting follow-up retain their source metadata.
...
any publication and delivery destination while preserving idempotency. Missing
or still-processing Zoom artifacts remain retryable.

Client credentials and token material must not appear in workflow inputs,
messages, repository state, infrastructure state, or runtime environment
variables. Live activation also requires Calendar read/event scopes and
explicit organizer aliases for managed meetings.
