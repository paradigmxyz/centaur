...
| `SESSION_CODEX_NANOCODEX_ROLLOUT_PERCENT` | `apiRs.codexNanocodexRolloutPercent`. | Percentage of API session requests for Codex assigned to Nanocodex. Assignment is deterministic by thread key and persisted as the session harness. Defaults to `50`; set to `0` to keep new Codex requests on Codex. |
