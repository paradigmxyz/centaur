...
| `SLACKBOTV2_NANOCODEX_ROLLOUT_PERCENT` | `slackbotv2.nanocodexRolloutPercent`. | Percentage of otherwise-default Codex threads assigned to Nanocodex. Assignment is deterministic by thread id; explicit flags and channel defaults win. Defaults to `50`; set to `0` to route all default traffic to Codex. |
