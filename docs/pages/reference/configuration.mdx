...
| `toolServer.gitCredentials.*` | Kubernetes Secret reference. | Direct-clone credential used only when repo-cache is disabled; generic credentials take precedence over `toolServer.githubToken`. |
...
iron-proxy. Centaur adds the clone URL's port to that proxy's public egress; a
literal IP such as `192.168.50.10` receives only an exact `/32` (IPv4) or `/128`
(IPv6) rule. Use a literal IP for a private-network direct clone. Private DNS
names should use repo-cache so the sandbox policy does not need broad,
DNS-dependent private-network access.
