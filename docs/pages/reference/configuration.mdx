...
| `apiRs.sessionOutputLineRetentionDays` / `SESSION_OUTPUT_LINE_RETENTION_DAYS` | Helm value, default `0` (disabled). | Deletes `session.output.line` events older than this many days. Harness stdout lines are the bulk of `session_events` and nothing removed them, so the table grows for the life of the deployment: about 450MB/day on a modest single node against a 20Gi default volume. Only output lines are deleted. Lifecycle events, execution outcomes and assistant replies are kept, so console transcripts stay intact. Output lines of a `queued` or `running` execution are kept until it finishes. The sweep reads through a partial index over output lines that migration 0055 builds concurrently at startup. Sweeps are bounded per pass, so a large first backlog drains over several. Retention caps growth. It does not shrink a volume that is already full. |
