...
| `CENTAUR_API_KEY` | `secretManager.existingSecretName`; local bootstrap generates it when absent. | Trusted Console key accepted by api-rs for all protected capabilities. Never expose it to sandboxes. |
...
| `CENTAUR_JWT_SIGNING_SECRET` | `secretManager.existingSecretName`; local bootstrap generates it when absent. | Signs Console-issued MCP tokens and sandbox api-rs principal tokens. |
