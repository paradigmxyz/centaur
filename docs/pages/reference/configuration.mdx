...
| `SLACKBOTV2_AGENT_VIEW_ENABLED` | `slackbotv2.agentViewEnabled`. | Enables Slack agent view, threaded DM sessions, native status and titles, and the Stop button. Defaults to `false`. Requires the Slack app migration and `agent_session_stopped` subscription described in [Quickstart](/quickstart). |
