...
| `SLACKBOTV2_AUTO_JOIN_CREATED_CHANNELS` | `slackbotv2.extraEnv`. | Joins newly-created public channels after subscribed `channel_created` events. Requires `channels:read` and `channels:join`. Defaults to `false`. |
