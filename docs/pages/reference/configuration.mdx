...
| `SLACKBOT_THREAD_FOLLOW` | `slackbot.extraEnv`. | Opt-in follow mode. Defaults off. When enabled, Slackbot also responds to 1:1 DMs and to replies in channel threads that already have an active Centaur runtime assignment. Requires the Slack app to subscribe to the relevant `message.*` events and scopes. |
