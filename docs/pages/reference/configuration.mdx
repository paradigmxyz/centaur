...
| `SLACKBOTV2_STEERING_REACTION_ENABLED` | `slackbotv2.steeringReactionEnabled`. | Reacts to mentioned follow-up messages while an execution is active, then removes the reaction after the steering handoff completes. Requires `reactions:write`. Defaults to `false` and disables itself for the running process if Slack reports the scope is missing. |
