...
| `CONSOLE_WORKFLOW_HARNESS` | `apiRs.extraEnv`, plus the same name in `SESSION_SANDBOX_PASSTHROUGH_ENV` so the workflow host inherits it. | Harness Console scheduled tasks run on (`claudecode`, `codex`, `amp`). Unset keeps the workflow-run default (`codex`); set it when the deployment's default harness is not codex, otherwise scheduled-task runs fail with the other harness's missing credential. |
