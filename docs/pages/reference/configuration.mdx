...
| `apiRs.sessionOutputLineRetentionDays` / `SESSION_OUTPUT_LINE_RETENTION_DAYS` | Helm value, default `0` (disabled). | Deletes `session.output.line` events older than this many days. Harness stdout lines are 98% of `session_events` rows and nothing removed them, so the table grows for the life of the deployment. One production deployment had reached 4.1GB at about 40k rows a day, against a 20Gi default volume. Only output lines are deleted. Lifecycle events, execution outcomes and assistant replies are kept, so console transcripts stay intact. Output lines of a `queued` or `running` execution are kept until it finishes. The sweep reads through a partial index over output lines that migration 0055 builds concurrently at startup. Sweeps are bounded per pass, so a large first backlog drains over several. Retention caps growth. It does not shrink a volume that is already full. |
| `apiRs.sessionOutputLineRetentionSweepIntervalSecs` / `SESSION_OUTPUT_LINE_RETENTION_SWEEP_INTERVAL_SECS` | Helm value, default `300`. | How often api-rs runs an output-line retention sweep when retention is enabled. |
