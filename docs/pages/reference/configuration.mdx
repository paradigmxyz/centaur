...
| `FIREWALL_MANAGER_SECRET_SOURCE` (discovery gate) | `ironProxy.secretSource`. | Under `env`, a tool is discovered only when all of its required `secrets` resolve to non-empty env vars; missing ones are skipped (`tool_skipped_missing_secrets`). No-op under `onepassword`/`onepassword-connect`. See [Tools](/extend/tools). |
