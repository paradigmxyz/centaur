...
| Env var or value | Set from | Controls |
| --- | --- | --- |
| `USER_MIGRATION_DIRS` | Chart-rendered from `overlays.sources[*].migrationsSubdir` (default `migrations`), or `apiRs.extraEnv`. | Colon-separated SQLx user migration directories. Applied once in an API background task before workflow initialization. Session readiness is independent of migration success. |
