...
| `SLACK_BOT_TOKEN` | Explicit `secretKeyRef` from `secretManager.existingSecretName`. | Slack Web API access for the api-rs Slack proxy, app-authored DMs, and workflow Slack helpers. The token remains server-side when callers use `slack.send_app_dm(...)`. |
