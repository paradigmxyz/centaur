...
| `GOOGLE_CHAT_CREDENTIALS` | Local shell before `just bootstrap-secrets`; production Secret. | Google service account JSON used by Googlechatbot when `googlechatbot.enabled=true`. |
| `GOOGLECHATBOT_API_KEY` | `secretManager.existingSecretName`; local bootstrap generates it when `GOOGLE_CHAT_CREDENTIALS` is present and it is omitted. | Static API key used by Googlechatbot. |
...
| `apiRs.sessionEventsRetentionDays` / `SESSION_EVENTS_RETENTION_DAYS` | Helm value, default `0` (disabled); range `0` through `3650`. | Deletes only `session.output.line` events older than this many days, once their execution also completed before the cutoff. Other session event types are preserved. Events without an execution expire by event age alone. Requires the [manual retention index](#stdout-event-retention). Sweeps skip deletion when the index is missing or invalid. |
...
| `SLACKBOTV2_AGENT_VIEW_ENABLED` | `slackbotv2.agentViewEnabled`. | Enables Slack agent view, threaded DM sessions, and native status and titles. Defaults to `false`. Requires the Slack app migration described in [Quickstart](/quickstart). Native Stop support is deferred. |
...
| Env var | Set from | Controls |
| --- | --- | --- |
| `PORT` | Runtime env. | Googlechatbot HTTP port; defaults to `3101`. |
| `LOG_LEVEL` | Runtime env. | Googlechatbot JSON log level: `debug`, `info`, `warn`, `error`, or `silent`. |
| `CENTAUR_API_URL` | Chart-rendered API service URL. | API base URL used by Googlechatbot; local default is `http://127.0.0.1:8080`. |
| `GOOGLECHATBOT_API_KEY` | `secretManager.existingSecretName`. | Dedicated API key used by Googlechatbot to call api-rs. It is scoped to `gchat:` sessions. |
| `GOOGLECHATBOT_DATABASE_URL`, `DATABASE_URL`, `POSTGRES_URL` | Secret. | Googlechatbot Postgres state store. The service refuses to boot without one unless tests/dev inject a state store. |
| `GOOGLECHATBOT_STATE_KEY_PREFIX` | Runtime env. | Postgres state namespace; defaults to `centaur-googlechatbot`. |
| `GOOGLE_CHAT_CREDENTIALS` | Secret. | Chat app service account JSON. `GOOGLE_CHAT_USE_ADC=true` uses Application Default Credentials instead; one of the two is required. |
| `GOOGLE_CHAT_PROJECT_NUMBER`, `GOOGLE_CHAT_ENDPOINT_URL` | `googlechatbot.projectNumber`, `googlechatbot.endpointUrl`. | Expected audience of the Google-signed webhook JWT, matching the Chat app's "Authentication audience" setting. At least one is required; the service refuses to start without one rather than accepting unverified requests. |
| `GOOGLE_CHAT_PUBSUB_TOPIC`, `GOOGLE_CHAT_PUBSUB_AUDIENCE`, `GOOGLE_CHAT_PUBSUB_SERVICE_ACCOUNT_EMAIL` | `googlechatbot.pubsub.*`. | Workspace Events delivery, used to receive every message in a space instead of only @mentions. The audience and the pushing service account identity are both required when a topic is set. |
| `GOOGLE_CHAT_BOT_USER_ID` | `googlechatbot.botUserId`. | `users/<id>` of the Chat app, so it recognizes its own messages and mentions in multi-bot spaces. |
| `GOOGLE_CHAT_IMPERSONATE_USER` | `googlechatbot.impersonateUser`. | Workspace user impersonated for Workspace Events subscriptions (domain-wide delegation). |
| `GCHAT_ALLOWED_SPACE_IDS`, `GCHAT_ALLOWED_DOMAINS`, `GCHAT_ALLOWED_SENDER_EMAILS` | `googlechatbot.allowedSpaceIds`, `googlechatbot.allowedDomains`, `googlechatbot.allowedSenderEmails`. | Comma-separated allowlists. Empty means Googlechatbot ignores all Google Chat messages. Space ids accept `spaces/AAAA` or `AAAA`. |
| `GCHAT_ALLOW_DIRECT_MESSAGES` | `googlechatbot.allowDirectMessages`. | Allows DM spaces; defaults to `false`. A DM also requires a sender allowlist, since it has no space to scope it by. |
| `GCHAT_REQUIRE_MENTION` | `googlechatbot.requireMention`. | Requires a bot mention before activating a thread; defaults to `true`. |
| `GCHAT_DEFAULT_HARNESS_TYPE` | `sandbox.harnessEngine`. | Default harness requested for new Google Chat sessions. Existing harness conflicts retry on the session's current harness. |
| `SESSION_IDLE_TIMEOUT_MS`, `SESSION_MAX_DURATION_MS` | Runtime env. | Forwarded to api-rs execute. `GCHAT_IDLE_TIMEOUT_MS` and `GCHAT_MAX_DURATION_MS` override these for Google Chat only. |
| `GCHAT_ACTIVE_EXECUTION_TTL_MS` | Runtime env. | Stale execution timeout used to unwedge Google Chat threads after crashes. |
| `GCHAT_RENDER_DELIVERY_TIMEOUT_MS` | Runtime env. | Timeout for Chat message create/update calls during rendering; defaults to 15 seconds. |
| `GCHAT_RENDER_MIN_EDIT_INTERVAL_MS` | Runtime env. | Minimum spacing between progress-message edits; defaults to `1500`. Google Chat has no streaming surface and rate limits `spaces.messages.update`. |
| `GCHAT_DOWNLOAD_ATTACHMENTS`, `GCHAT_ATTACHMENT_MAX_BYTES` | `googlechatbot.downloadAttachments`, runtime env. | Enables Chat attachment downloads into base64 payloads (default `false`) and the size cap (default 10 MiB). |
...
Retention is disabled by default. Its index is installed separately from API-RS
startup migrations, so an index build failure does not prevent the API from
starting. Create the index before enabling retention.
...
The script creates a partial index concurrently. Run it outside a transaction;
...
Expect one row with all three flags set to `true`. Then set
...
If a build fails, inspect the index before retrying. No row means the index is
absent and the creation script can be retried after addressing the failure.
If the index exists but is invalid, repair it outside a transaction:

```sql
REINDEX INDEX CONCURRENTLY session_events_stdout_created_at_idx;
```

Verify the flags again after repair. The creation script deliberately fails if
the name already exists, so a retry cannot silently accept an invalid index.
An already valid index does not need to be recreated.
