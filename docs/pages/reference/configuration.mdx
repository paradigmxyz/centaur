...
| `SLACKBOTV2_QUOTA_FALLBACK_HARNESS` | `slackbotv2.quotaFallbackHarness`. | Alternate to `sandbox.harnessEngine` after an execution fails with the `quota` failure class. Threads on the default move to this harness; threads already on this alternate move back to the default. The sticky switch replays once per message and does not override an explicit harness flag on that message. Empty disables automatic fallback. |
