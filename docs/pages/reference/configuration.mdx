...
| `SLACK_SIGNING_SECRET` | `secretManager.existingSecretName`; local bootstrap reads shell env. | Slack request signature verification. Optional when `slackbotv2.socketMode=true`. |
...
| `SLACK_APP_TOKEN` | `secretManager.existingSecretName`; local bootstrap patches from the shell when present. | App-level token (`xapp-...`, `connections:write` scope) for the Slackbot's Socket Mode connection. Required when `slackbotv2.socketMode=true`. |
