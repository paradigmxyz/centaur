...
| `apiRs.nodeSelector`, `apiRs.tolerations` | Places api-rs pods on selected nodes, including dedicated tainted node pools. |
| `console.nodeSelector`, `console.tolerations` | Places Console web pods on selected nodes, including dedicated tainted node pools. |
| `console.worker.nodeSelector`, `console.worker.tolerations` | Places Console background worker pods independently from the web pods. |
| `postgres.nodeSelector` | Places the bundled PostgreSQL pod on selected nodes. Ensure the node pool can mount the configured persistent storage. |
