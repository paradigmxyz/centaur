...
| `SLACKBOTV2_QUOTA_FALLBACK_HARNESS` | `slackbotv2.quotaFallbackHarness`. | Harness a thread is restarted onto (sticky, once per message) after an execution fails with the `quota` failure class — e.g. `claudecode` to ride out a Codex credit outage on Claude Code. Empty disables automatic fallback. |
