...
| `SLACKBOTV2_AUTO_JOIN_CREATED_CHANNELS` | `slackbotv2.autoJoinCreatedChannels`. | Enables Slackbot v2 to join newly-created public channels after `channel_created` events. Requires the Slack app to subscribe to `channel_created` and grant `channels:read` plus `channels:join`. Defaults to `false`. |
