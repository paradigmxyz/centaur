...
| `apiRs.sessionEventsRetentionDays` / `SESSION_EVENTS_RETENTION_DAYS` | Helm value, default `0` (disabled). | Deletes only `session.output.line` events older than this many days. Other session event types are preserved. Events belonging to a `queued` or `running` execution are never deleted, whatever their age. Sweeps are bounded per pass, so a large first backlog drains over several. |
