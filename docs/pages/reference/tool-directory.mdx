...
| `meeting-scheduler` | Google Calendar and Zoom booking, reconciliation, recording, and summary retrieval | `GOOGLE_TOKEN_JSON`, `ZOOM_ACCESS_TOKEN`, `CENTAUR_POSTGRES_DSN` |
...
| `meeting-scheduler` | Calendar/Zoom booking and post-meeting artifact retrieval | `GOOGLE_TOKEN_JSON`, `ZOOM_ACCESS_TOKEN`, `CENTAUR_POSTGRES_DSN` |
...
| `docsend` | Download DocSend documents through browser automation | `BROWSERBASE_API_KEY` |
...
| `eventregistry` | Search recent articles with Event Registry | `EVENTREGISTRY_API_KEY` |
