...
| `websearch` | Web search over Tako's data graph and the web (default) or Parallel, plus deep research | None for `search`; `TAKO_API_KEY` for Tako REST and `deep-research`; `PARALLEL_API_KEY` when `WEBSEARCH_BACKEND=parallel`; `ANTHROPIC_API_KEY` for search synthesis |
