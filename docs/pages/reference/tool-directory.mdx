...
The Slack tool exposes two DM sender identities. `slack dm USER MESSAGE` uses
the caller's injected Slack credential. `slack app-dm USER MESSAGE` sends as the
deployment's Slack app through api-rs and does not require a personal Slack
credential.
