...
| `websearch` | Web search and deep research via Parallel (default) or Tako's data graph and the web | None for `search`; `PARALLEL_API_KEY` for Parallel REST and `deep-research`; `TAKO_API_KEY` when `WEBSEARCH_BACKEND=tako`; `ANTHROPIC_API_KEY` for search synthesis |
