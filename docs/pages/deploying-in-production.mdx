...
7. Subscribe to `app_mention` and to the message events you want Centaur to see:
   `message.channels`, `message.groups`, and `message.im`. Subscribe to
   `channel_created` to automatically join newly-created public channels.
...
`block_actions` interactions. It also joins newly-created public channels from
`channel_created` events when that event is subscribed. The Slack app needs
`channels:read` to receive those events and `channels:join` for the auto-join
behavior. Do not rely on assistant-specific Slack event types unless the
Slackbot code has explicit support for them.
