...
3. Set `slackbotv2.socketMode: true` and `ingress.enabled: false`. The
   Request URLs can be removed and `SLACK_SIGNING_SECRET` becomes optional;
   webhook routes stay in place but go unused.
