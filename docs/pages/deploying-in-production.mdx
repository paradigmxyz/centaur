...
| `CENTAUR_API_KEY` | Console to API | Trusted static service token with all protected api-rs capabilities. Do not mount it into sandboxes. |
...
| `CENTAUR_JWT_SIGNING_SECRET` | Console-issued MCP and sandbox api-rs tokens | Generate at least 64 random bytes. |
...
Generate distinct high-entropy values for `CENTAUR_API_KEY` and each enabled
ingress service key. Store them in the infra Secret. api-rs rejects duplicate
configured tokens at startup.
...
CENTAUR_API_KEY=$(kubectl get secret -n centaur-system centaur-infra-env \
  -o jsonpath='{.data.CENTAUR_API_KEY}' | base64 --decode)
