...
Slackbot keeps strict mention-only behavior by default. Set
`SLACKBOT_THREAD_FOLLOW=1` only after the app has the message subscriptions and
history scopes for the surfaces you want enabled. Follow mode responds to 1:1
DMs and to replies in channel threads with an active Centaur runtime assignment;
unrelated channel messages remain ignored.
