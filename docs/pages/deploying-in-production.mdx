...
7. Subscribe to `app_mention` and to the message events you want Centaur to see:
   ...
   [Slack app migration](/quickstart), set `slackbotv2.agentViewEnabled` to `true`,
   and subscribe to `agent_session_stopped` so the native Stop button interrupts
   execution. The toggle defaults to `false`.
