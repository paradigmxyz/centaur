...
| `SLACK_SIGNING_SECRET` | Slackbot/API | Used to verify Slack webhook signatures. Optional in Socket Mode. |
| `SLACK_APP_TOKEN` | Slackbot (Socket Mode) | App-level token (`xapp-...`, `connections:write`) when `slackbotv2.socketMode=true`. |
...
| Codex with a custom provider | `codex` | `--provider <id>` | Provider `apiKeyEnv` | Provider `baseUrl` host |
...
To register a private OpenAI-compatible Responses provider, add it once under
...
Store the named key in the configured iron-proxy secret source. The chart sends
the non-secret provider catalog to api-rs, sandboxes, chat ingresses, and the
Console. api-rs registers a bearer-token replacement restricted to the
`baseUrl` DNS host; sandboxes receive only the `apiKeyEnv` placeholder. Select
the provider with `--provider private_responses`; `--model` overrides its
configured default for that turn. If `defaultModel` is omitted, callers must
pass `--model`; the Console lists only custom providers that define a default.
...
If the deployment has no public URL, run the Slackbot on an outbound Socket
Mode connection instead of webhooks:

1. In the Slack app, enable **Socket Mode** and generate an **app-level token**
   (`xapp-...`) with the `connections:write` scope.
2. Store the token as `SLACK_APP_TOKEN` in the infra Secret.
3. Set `slackbotv2.socketMode: true`. The Request URLs can be removed and
   `SLACK_SIGNING_SECRET` becomes optional.

The chart drops the Slackbot from the shared ingress on its own, so leave
`ingress.enabled` alone unless nothing else needs it: the same ingress carries
the GitHub and Linear webhook paths, and turning it off takes those bots down
with it.

Event subscriptions are unchanged. Socket Mode is single-consumer (one
connection per app-level token), so keep `slackbotv2.replicaCount: 1`. Slack
does not redeliver dropped socket events the way it redelivers failed webhook
POSTs, so the Slackbot connects only once its database is up. A socket that is
down — a rejected app-level token, or one that never reconnects — fails the
`/health` probe instead of leaving the pod up without a connection.
