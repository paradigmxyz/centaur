...
7. Subscribe to `app_mention` and to the message events you want Centaur to see:
   `message.channels`, `message.groups`, and `message.im`. If
   `SLACKBOTV2_AUTO_JOIN_CREATED_CHANNELS` is enabled, also subscribe to
   `channel_created`.
...
enabled, it also joins newly-created public channels from `channel_created`
events. The Slack app needs `channels:read` to receive those events and
`channels:join` for the auto-join behavior. Do not rely on assistant-specific
Slack event types unless the Slackbot code has explicit support for them.
