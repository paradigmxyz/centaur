...
subscribe to `app_mention` and the `message.*` events you want. Make sure the
bot has the `chat:write` scope for Slack's streaming API. If
`SLACKBOTV2_AUTO_JOIN_CREATED_CHANNELS` is enabled, also subscribe to
`channel_created` and grant `channels:read` plus `channels:join` so Slackbot v2
can join newly-created public channels.
