...
By default, Slackbot responds only to mentions. To let it also answer 1:1 DMs
and follow replies in channel threads that already have an active Centaur runtime
assignment, set `SLACKBOT_THREAD_FOLLOW=1` through `slackbot.extraEnv` and
subscribe the Slack app to the matching message events, such as
