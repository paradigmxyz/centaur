...
accepts in the `PRIVATE-TOKEN` header for `GET /api/v3/projects`. The Token must
...
When the repository picker returns `repository_catalog_unavailable`, inspect
the api-rs log entry whose event is
`gitlab_repository_catalog_unavailable`. Its fixed `failure_code` distinguishes
`unexpected_redirect`, `authentication_rejected`, `connection_failed`,
`request_timeout`, and `invalid_response`; an upstream HTTP status may also be
present. The entry deliberately excludes the request URL, search text, response
body, and Token.
