...
`SLACKBOT_API_KEY` is Slackbot's dedicated static service token. Bootstrap also
generates `CENTAUR_API_KEY`, the separate trusted key shared by Console and
api-rs, when it is absent.
