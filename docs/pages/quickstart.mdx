...
Subscribe to the `app_mention` bot event. If `SLACKBOTV2_AUTO_JOIN_CREATED_CHANNELS`
is enabled, also subscribe to `channel_created`. For a minimal channel-mention
test, the app needs Bot Token Scopes that let it read mentions and write
replies, for example `app_mentions:read` and `chat:write`. Auto-joining new
public channels also requires `channels:read` and `channels:join`. If you enable
DM events such as `message.im`, Slack will also require direct-message scopes
such as `im:history`.
