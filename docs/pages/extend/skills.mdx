...
centaur-skills read builtin:incident-response
centaur-skills read skl_example
centaur-skills list --scope shared
```

`builtin:` references read from the sandbox filesystem. `skl_` IDs read from
Console through iron-proxy using the sandbox's existing entitlement token. A
skill never adds credentials or grants.
