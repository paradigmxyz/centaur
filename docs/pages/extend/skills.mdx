...
Agents can search both sources through the catalog command:

```bash
centaur-skills search "investigate a failed rollout"
centaur-skills read incident-response
centaur-skills read skl_example
centaur-skills list --scope shared
```

Builtin skill names read from the sandbox filesystem. `skl_` OIDs read from
Console through iron-proxy using the sandbox's existing entitlement token.
Names use lowercase letters, numbers, and hyphens, so they cannot conflict with
the underscore in a Console OID. A skill never adds credentials or grants.
