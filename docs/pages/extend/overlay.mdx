...
`workflowsSubdir: workflows`, `migrationsSubdir: migrations`, and
`skillsSubdir: .agents/skills`. Directories a
...
Overlays can include SQLx migration files in `migrations/`. The API applies
these in a background task at startup before starting workflow workers.
Session routes remain available while migrations run or if they fail;
workflow routes return HTTP 503 until initialization succeeds.

Migrations from every overlay share one version sequence and a history table
in `user_migrations`. Versions must be unique across overlays. Unlike tools
and workflows, migration files never shadow each other. SQL files explicitly
create and reference application schemas. See [Workflow Databases](./workflows#workflow-databases)
for an example and retry behavior.

Set `migrationsSubdir` to customize the directory or to `""` to disable it.
Pin overlay revisions for deployments that change the database, and restart
the API to apply new migrations. A repo-cache refresh does not rerun them.
...
|---------|-------|----------|
| API | `/var/lib/centaur/repos/<owner>/<repo>` | Tool-secret discovery, workflow discovery, and user migrations. |
