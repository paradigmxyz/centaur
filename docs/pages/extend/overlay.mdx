...
rejects URLs containing user-info and supplies credentials through
`GIT_ASKPASS` from the mounted Secret.
...
repository access. It does not open a broad custom-port rule for a DNS name;
use repo-cache for private DNS remotes or DNS remotes on non-HTTPS ports.
