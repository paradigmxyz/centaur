...
An unauthorized click can return without updating the message, leaving its
buttons available to an authorized user. The receiving workflow must update
or remove buttons explicitly. There is no automatic expiration or cancellation
...
Lu.ma webhooks use a timestamped HMAC signature. Configure them with
`{"type": "luma", "secret_ref": "LUMA_WEBHOOK_SECRET"}`; Centaur verifies
`Webhook-Signature` against `Webhook-Timestamp` and rejects deliveries outside
