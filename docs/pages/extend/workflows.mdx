...
Workflows can own PostgreSQL schemas and run their own SQL migrations. Call
`ctx.db.migrate()` when the handler needs its tables. Migration execution is
explicit: discovery and host startup do not apply migrations.
...
```sql
-- migrations/001_checkpoints.sql
CREATE TABLE sample_index.checkpoints (
...
async def handler(inp, ctx):
    applied = await ctx.db.migrate("./migrations", schema="sample_index")
...
Relative migration paths resolve from the workflow module. The helper creates
the schema and a reserved `_workflow_migrations` table inside it. The schema
name identifies the migration history, so moving or renaming a workflow does
not reset its history. Workflows sharing a schema must share one authoritative
migration directory.

Migration files are UTF-8 SQL named `<positive_version>_<description>.sql`.
Versions sort numerically and need not be consecutive. The helper returns the
filenames applied by that call, or an empty list when nothing is pending. An
empty or missing migration directory is an error. The history records the
version, filename, and application time. Applied versions are skipped even if
their SQL content changes. Add a new numbered migration to make a change. Do
not rename applied files or insert new files before an already-applied version.

Each file runs in a transaction together with its history entry. Failure rolls
back that file and fails the workflow execution; earlier successful files stay
applied. Retrying resumes with the pending files. A database advisory lock
serializes migrations for the same schema. Other schemas can migrate
independently. Lock acquisition times out after 60 seconds by default; pass
`lock_timeout` in seconds to change it. Cancellation and connection loss
release the lock through connection cleanup.

Use ordinary SQL with schema-qualified table names. The migrator does not
change `search_path`. PostgreSQL executes each file through a PL/pgSQL `DO`
block with dynamic SQL, which supports multiple statements and function
definitions but rejects transaction-control statements. Do not include
`BEGIN`, `COMMIT`, `ROLLBACK`, psql commands, or operations that require running
outside a transaction, such as `CREATE INDEX CONCURRENTLY`. Keep migration SQL
focused on database changes, and do not change the migration ledger or advisory
locks from a migration.

Call `migrate()` on every handler invocation, outside `ctx.step()`. Its database
history supplies retry safety. Older workflow bundles may run against a newer
database when their migration versions and filenames match the recorded
history. Keep changes compatible with executions using older code. Missing files within a
bundle's version range are errors; newer applied versions beyond that range
are left in place. There is no automatic downgrade.

`ctx.db` also exposes `execute`, `fetch`, `fetchrow`, `fetchval`, and `acquire`
using the existing asyncpg pool. Queries are ordinary database operations and
are not automatically checkpointed. For multiple queries in one transaction:
...
Use durable steps and idempotent application writes where retries can repeat
side effects. Advance an indexer's cursor only after processing succeeds.

Database access uses the host's configured `DATABASE_URL` and that role's
existing privileges. Schemas do not automatically grant or restrict access.
The role needs permission to create the target schema, or an existing schema
with permission to create and alter its objects. Workflows without database
operations do not need a database connection. Existing `ctx._pool` calls remain
supported.

To run database-backed host tests, point `WORKFLOW_DATABASE_TEST_URL` at a
disposable local PostgreSQL database, then run:
