...
Cancelling the workflow closes its pending buttons. Click context includes
...
The callback result is checkpointed. `ctx.start_workflow(..., name="...")`
also checkpoints the child start and derives a stable idempotency key, so a
retry cannot enqueue another child for that named step.
Callback return values must be JSON-compatible.

Slack delivery retries separately from the recorded click. The API updates the
message with the selected choice and user, or with expiry/cancellation status.
...
See `workflows/button_actions.py` for an example with Approve starting an echo
workflow and Reject returning the clicking user.
