...
Slackbot fills `click` from the verified Slack interaction, overriding any
`click` field in the button's input. It contains `id` (the button group ID),
...
The serialized workflow name and input must fit within 2000 bytes. Pass record
...
button endpoint. Workflow-start permissions still govern ordinary API callers.
