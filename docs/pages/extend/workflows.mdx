...
## Run Actions From Slack Buttons

Use `ctx.slack_actions` to post buttons and run a Python callback when an
authorized user clicks one. The workflow suspends durably while waiting.
Callbacks execute inside the same workflow and can call tools, run agents,
start child workflows, or return a value.
...
async def handler(inp, ctx):
    async def approve(click):
        return await ctx.start_workflow(
            "echo",
            {"approved_by": click.user_id},
            idempotency_key=f"approval:{click.id}",
        )
    ...
    result = await ctx.slack_actions(
        ...
        channel=inp["channel"],
        team_id=inp["team_id"],
        text="Proceed with this request?",
        allowed_users=inp["approver_ids"],
        buttons={
...
Each group is single-use. The first permitted click selects one callback.
Subsequent clicks cannot select another callback. For repeated interactions,
create another group with a different step name. Groups support one to five
buttons, explicit Slack user IDs, and timeouts from one second to 30 days.
The timeout starts when the group is first created and does not reset on replay.
The team and channel must match the posted message. Button IDs use letters,
digits, underscores, or hyphens. Labels contain at most 75 characters.

The result contains `outcome`, `click`, and `value`. A selected button returns
`outcome="clicked"`, verified click context, and the callback's return value.
An expired request returns `outcome="expired"` without calling a callback.
Cancelling the workflow prevents further button actions, but may leave buttons
visible in Slack. Click context includes
`id`, `action`, `user_id`, `team_id`, `channel_id`, `message_ts`, and `action_ts`.

Keep group names stable and unique within the workflow. The API persists each
group's configuration, audience, deadline, and winning click. Replaying with
different configuration fails explicitly. Callbacks follow ordinary workflow
replay rules: keep their meaning compatible for outstanding requests, use
durable steps, and make external side effects idempotent. A callback can retry
after failure; a recorded click does not make an external API call exactly once.
The callback result is checkpointed. When starting a child workflow, pass a
stable `idempotency_key`, such as `f"approval:{click.id}"`, so a retry cannot
enqueue another child. Use a different key for each distinct child start.
Callback return values must be JSON-compatible.

Slack posting and message updates use the owning workflow's checkpoints and
retry policy. After a click or timeout, the workflow updates the message before
running the selected callback or returning the expired result. A Slack failure
can delay callback execution. There is no background message cleanup after a
workflow is cancelled or exhausts its retries. Further clicks are rejected.
An ambiguous Slack posting timeout can produce duplicate messages; all copies
reference the same single-use group. Feedback says the selection was recorded,
not that the callback's work has finished.

This uses the existing Slackbot interactivity endpoint and API Slack bot token.
Slackbot needs its dedicated `SLACKBOT_API_KEY`; a sandbox principal cannot
submit another user's click. Existing custom Slack action IDs continue to emit
raw workflow events. The `centaur.workflow.action:` namespace is reserved for
these buttons. Internal action wakeups cannot be emitted through the public
workflow event API. `SLACK_API_URL` can target a local Slack emulator in both
the API and Slackbot during development.
