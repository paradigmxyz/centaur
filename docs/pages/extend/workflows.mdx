...
async def handler(inp, ctx):
    applied = await ctx.db.migrate("./sample_indexer_migrations", schema="sample_index")
    row = await ctx.db.fetchrow(
        "SELECT cursor FROM sample_index.checkpoints WHERE source_id = $1",
        inp["source_id"],
    )
    return {"applied": applied, "cursor": row["cursor"] if row else None}
```

Relative migration paths resolve from the workflow module. The Python helper
invokes the bundled `centaur-workflow-migrate` binary, which uses the same SQLx
migration engine as the Rust control plane. The runner creates the requested
schema if needed and sets its dedicated connection's `search_path` to that
schema only. SQLx stores history in that schema's `_sqlx_migrations` table.
The control plane's migration history is separate.
...
the numeric versions applied by that call, or an empty list when nothing is
pending. Application queries still use the host's existing pool and should
qualify table names with the schema.
...
The runner only applies forward migrations; it does not automatically downgrade.
...
The runner uses SQLx's database-wide advisory lock, including during schema
creation. Migrations for different schemas serialize while they run. Waiting
for this lock times out after 60 seconds by default; pass `lock_timeout` in
seconds to change it. This timeout does not limit the migration SQL itself.
Cancellation terminates the runner, and the runner exits if its host pipe
closes. On PostgreSQL 14 or newer, the runner enables one-second connection
checks so the server can detect a disconnected runner during a long query.
Closing its dedicated database connection releases migration locks.
...
Both the runner and application pool use the host's `DATABASE_URL`. The role
...
Both runtime images include the runner. For local development, build it with
`cargo build --manifest-path services/api-rs/Cargo.toml -p centaur-workflow-migrate`
and set `WORKFLOW_MIGRATOR_PATH` to the resulting executable, or put it on `PATH`.
