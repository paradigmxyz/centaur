...
Workflows use `ctx.db` to query the host's PostgreSQL database. Database schema
changes live in deployment overlays, separate from workflow definitions.
Single-file workflows and workflow packages work the same way.

```text
example-overlay/
  migrations/
    202609080001_create_checkpoints.sql
  workflows/
    sample_indexer.py
```

```sql
-- migrations/202609080001_create_checkpoints.sql
CREATE SCHEMA sample_index;
...
The API loads user migrations once at startup in an async background task,
after its core migrations. Session routes remain available while this task
runs. Workflow workers, schedules, and discovery start after migration success.
Workflow routes return HTTP 503 until initialization succeeds. A migration
failure keeps workflows unavailable and writes the error to API logs without
stopping the session API. Fix the migration and restart to retry.

The chart discovers `migrations/` in each overlay. Set `migrationsSubdir` to
relocate it or to `""` to disable it for a source. An overlay without that
directory is allowed; an unavailable parent checkout fails initialization.
For local development, set `USER_MIGRATION_DIRS` to a colon-separated list of
directories. An unset variable disables user migrations. User migrations run
independently of `RUN_MIGRATIONS`, which controls core migrations only.

SQLx combines files from all configured directories into one sequence, sorted
by numeric version. Use `<version>_<description>.sql` filenames, with unique
versions across overlays. Timestamp versions make coordination easier.
Duplicate versions fail instead of shadowing files from another overlay.
Applied migrations must not be edited: SQLx checks their checksums. Reversible
SQLx file pairs are supported, but startup applies only forward migrations.

History lives in `user_migrations._sqlx_migrations`, separate from the core
migration table. SQL files should explicitly create their application schemas
and qualify table names. The dedicated migration connection's `search_path`
contains only `user_migrations`; application pool connections keep their
existing settings. Schemas do not automatically restrict permissions.

SQLx's database-wide advisory lock serializes migration runs across API
replicas. Each migration and its history entry normally commit in one
transaction. A failure rolls back that file while earlier files stay applied.
SQLx's `-- no-transaction` directive is supported, but those files can leave
partial changes on failure. Do not change migration history, advisory locks,
`search_path`, or transaction boundaries in migration SQL. This path requires
PostgreSQL 14 or newer for connection-loss detection during long queries.

Already-running replicas continue executing workflows during a rollout.
Keep schema changes compatible with them. Applied versions absent from an
older replica's overlay are allowed. Pin overlay revisions and restart the API
when adding migrations: repo-cache refreshes alone do not rerun migrations.

`ctx.db` exposes `execute`, `fetch`, `fetchrow`, `fetchval`, and `acquire` through
the existing asyncpg pool. Queries are not automatically checkpointed. Use
durable steps and idempotent writes for side effects, and advance an indexer's
cursor only after processing succeeds. For multiple queries in one transaction:
...
Both migrations and workflow queries use the configured `DATABASE_URL`.
Existing `ctx._pool` calls remain supported. Workflows without database
operations do not need a database driver installed in the Python host.
