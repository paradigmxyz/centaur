...
- Tests run from repo root: `uv run --no-project --with pytest --with "tako-sdk>=2.2.6" --with python-dotenv --with typer --with rich python -m pytest tools/research/tako/test_client.py -v`. The test file inserts repo root into `sys.path` (same pattern as `tools/research/youtube/test_client.py`) so `centaur_sdk` (repo root package) and `tools.research.tako.*` import.
