...
use harness_server::{
    ...
    run_nanocodex_blocks_server, run_omp_blocks_server, run_validate_agent_deltas,
...
enum CliCommand {
    ...
    Hermes,
    /// Run OMP RPC with native session continuation over blocks input.
    Omp,
    ValidateJsonrpc,
    ValidateAgentDeltas,
}
...
fn run() -> Result<()> {
    match Cli::parse()
        ...
        CliCommand::Omp => run_omp_blocks_server(),
