...
|Use `centaur-skills search "<task>"` when specialized guidance would materially help and no skill already listed for the current session clearly applies. Read the best match with `centaur-skills read <skill-identifier>` before following it.
|The skill catalog includes deployment-managed skills identified by name plus Console skills identified by `skl_` OIDs. Console applies private and public visibility rules for the current principal. Catalog results are instructions only and never expand the current principal's tool or credential grants.
...
|centaur-skills search "task"    → discover relevant builtin, private, and shared skills
|centaur-skills read <name-or-oid> → read the complete current SKILL.md for a catalog result
