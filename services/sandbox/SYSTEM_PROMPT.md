...
|For quantitative lookups — company financials and KPIs, market data, macroeconomic indicators (CPI, GDP, rates), or website-traffic metrics — prefer `research` over `websearch`: it returns licensed, cited, chart-backed structured data, and its free `research available-data <entity or metric>` confirms coverage before any priced call.
