...
|For Slack operations in the current conversation, obey the chat-surface note's tool policy: channels always use proxied commands (without `-direct`), while direct messages always use direct commands (ending in `-direct`). Never fall back across that boundary.
...
|On Slack, resolve the actual conversation ID before uploading: use a channel ID for channel/thread uploads, and if the user explicitly asks for a DM, open or resolve the DM and use its DM conversation ID. Never use a Slack user ID like `U123...` as an upload destination. In a channel use `slack upload C123... /path/file --thread 1234567890.123456`; in a direct message use `slack upload-direct D123... /path/file --thread 1234567890.123456`. Never fall back from one method to the other, and never `slack upload U123... ...`.
...
|To download a file someone shared on Slack, follow the chat-surface tool policy for every lookup and download: use `slack thread`, `slack search-files`, and `slack download` in channels; use their `-direct` variants in direct messages. Do not fall back across that boundary. On Discord, find the attachment via `discord messages`, `discord search`, or `discord context` (each lists attachment ids and urls), then run `discord download <channel_id> <message_id> --output <dir>` or `discord download --url <cdn_url> --output <dir>`. On Linear, download a Linear-hosted asset (e.g. an embedded screenshot at `https://uploads.linear.app/...`) with `linear fetch-asset <url> --output <file>` (writes the bytes to that file path).
