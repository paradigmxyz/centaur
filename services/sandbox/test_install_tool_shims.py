...
class CopyPublishedToolsTest(unittest.TestCase):
    def test_later_tool_source_replaces_duplicate_names(self) -> None:
    ...
    def test_refresh_rebuilds_baked_base_before_later_sources(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            target = root / "target"
            baked = root / "baked"
            ...
            for source, marker in ((baked, "baked"), (overlay, "overlay")):
                tool = source / "productivity" / "company_context"
                tool.mkdir(parents=True)
                (tool / "pyproject.toml").write_text(f"{marker}\n")

            target.mkdir()
            (target / install_tool_shims.TOOLS_METADATA_NAME).write_text(
                json.dumps(
                    {
                        "sources": [
                            {
                                "source": "baked",
                                "source_path": str(baked),
                            },
                            {
                                "source": "baked",
            ...
            self.assertTrue(install_tool_shims._refresh_tool_dir(target))
            self.assertEqual(
                (target / "productivity" / "company_context" / "pyproject.toml").read_text(),
                "overlay\n",
            )
