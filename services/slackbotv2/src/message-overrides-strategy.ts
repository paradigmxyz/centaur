...
const SYSTEM_PROMPT = [
  ...
  'Map Amp model aliases to canonical IDs: deep -> deep, fast -> fast. Select an Amp model only when the user explicitly names Amp or clearly asks for the deep or fast model/mode. Do not infer Amp from words about intelligence, thoroughness, speed, or task style.',
  'Words containing or evoking model aliases are not model requests. For example, "use the gigabrain", "think deeply", "do a deep analysis", and "give me a fast answer" do not select Amp. Unless another explicit selector is present, "use the gigabrain" returns null for every field.',
