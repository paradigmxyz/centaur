...
export type GoogleChatbotLogLevel = 'debug' | 'info' | 'warn' | 'error' | 'silent';

const LOG_LEVELS: readonly GoogleChatbotLogLevel[] = ['debug', 'info', 'warn', 'error', 'silent'];

export function normalizeGoogleChatbotLogLevel(value: string | undefined): GoogleChatbotLogLevel {
  const normalized = value?.toLowerCase();
  return LOG_LEVELS.includes(normalized as GoogleChatbotLogLevel)
    ? normalized as GoogleChatbotLogLevel
    : 'info';
}

export function createGoogleChatbotLogger(minLevel: GoogleChatbotLogLevel): Logger {
  const logger: Logger = {
...
function log(
  minLevel: GoogleChatbotLogLevel,
  level: Exclude<GoogleChatbotLogLevel, 'silent'>,
  event: string,
  data?: unknown,
): void {
