...
async fn initialize_runtime(args: Args, app_state: AppState) -> Result<(), ServerError> {
    ...
    if let Some((manager, credential_ref, reconcile_interval)) = args.workspace_manager().await? {
        runtime = runtime.with_workspace_manager(manager.clone(), credential_ref.clone());
        if args.gitlab_publishing_enabled() {
            runtime = runtime.with_gitlab_publisher(manager, credential_ref);
